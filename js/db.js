 ...
 */
const DB = (() => {
  const KEYS = { settings: 'hd_settings', auth: 'hd_auth' };
  const SESSION_KEY = 'hd_session';
  const DEFAULT_PASSWORD = '    ';
  const DEFAULT_SETTINGS = {
    hero_subtitle: ' Game Developer · iOS',
    about_bio: "Hi, I'm Imad — a passionate game developer building fun iOS experiences under the HarouDev brand.",
  ...
  function exportAll() { return JSON.stringify({ [KEYS.settings]: _read(KEYS.settings), [KEYS.auth]: _read(KEYS.auth) }, null, 2); }
  ...
  _ensurePassword();
  return { getSettings, saveSettings, setPassword, checkPassword, isLoggedIn, login, logout, exportAll, importAll };
})();
